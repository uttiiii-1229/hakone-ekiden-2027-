  ...
  for(let attempt=1;attempt<=3;attempt++){
    try{
      ...
      const res=await fetch(url,{signal:controller.signal,headers:{
        'user-agent':'Mozilla/5.0 (compatible; UniversityEkidenDatabase/1.0)',
...
const eventCount=Object.values(db.meets).reduce((n,m)=>n+Object.keys(m.events||{}).length,0);
if(meetCount===0||eventCount===0){
  console.error('No usable meet results fetched; preserving existing DB.');
  process.exitCode=2;
} else {
  ...
  console.log('wrote',outputPath,meetCount,'meets',eventCount,'events');
}
