// Current-athlete PB audit: Waseda University — 2026-09-21
// Source: Waseda University Track & Field official athlete profile / competition results.
// Only officially confirmed PBs are applied. Missing values are never inferred.
(() => {
  const audits={
    '早稲田大学':{
  ...
  resolver.currentRows=function(currentTeam){
    ...
    Object.entries(pb).forEach(([name,v])=>{
      let row=byName.get(norm(name));
      if(!row && team==='早稲田大学' && norm(name)===norm('増子 陽太')){
        row={name,grade:'1',pb5000:v[0],pb10000:v[1],half:v[2],sources:['Waseda official 2026 roster/PB']};
        rows.push(row);byName.set(norm(name),row);return;
      }
      if(!row)return;
