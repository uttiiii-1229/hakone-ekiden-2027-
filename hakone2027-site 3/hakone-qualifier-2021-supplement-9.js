...
(() => {
  ...
  for (const row of rows) {
    const found = existing.individuals.find(x => norm(x.name) === norm(row.name) && norm(x.team) === norm(row.team) && norm(x.status) === norm(row.status));
    if (!found) existing.individuals.push(row); else for (const field of ['time','name','team','grade','status','note']) if (!hasValue(found[field]) && hasValue(row[field])) found[field] = row[field];
  }
  existing.individuals.sort((a,b) => { const ar=Number(a.rank),br=Number(b.rank); const av=Number.isFinite(ar)&&ar>0?ar:9999; const bv=Number.isFinite(br)&&br>0?br:9999; return av-bv; });
  ...
  if (!document.querySelector('script[data-hakone-q-2022-s1]')) {
    const s=document.createElement('script'); s.src='hakone-qualifier-2022-supplement.js'; s.dataset.hakoneQ2022S1='1'; document.head.appendChild(s);
  }
})();